...
data_m_2 <- data_use %>% 
   mutate(permit_type = substr(provider_type_cd, 1, 2))
```
...
```{r}
# West Coast
data_m_pacific <- data_m_2%>%filter(state_cdstr==c("CA","WA","OR"))
```
...
ggplot(data_m_pacific, aes(x = state_cdstr, fill = permit_type)) +
...
ggplot(data_m_northeast, aes(x = state_cdstr, fill = permit_type)) +
  geom_bar(position = "dodge") +
  labs(title = "Distribution of Providers by State",
       x = "State",
       y = "Count",
...
ggplot(data_m_midwest, aes(x = state_cdstr, fill = permit_type)) +
  geom_bar(position = "dodge") +
  labs(title = "Distribution of Providers by State",
       x = "State",
       y = "Count",
...
ggplot(data_m_pacific, aes(x = state_cdstr, fill = permit_type)) +
  ...
  labs(title = "Distribution of Providers by State",
       ...
       fill = "Provider Type") +
  #facet_wrap(~state_cdstr, scales = "free_y") +
  facet_wrap(~permit_type, scales = "free_y") +
  theme_minimal()
